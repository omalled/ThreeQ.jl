...
#output: heads
function forwardmodel(ks::Array{Float64, 1}, rs::Array{Float64, 1})
...
function forwardmodel(kb, rs, klow, khigh)
	ks = b2f(kb, klow, khigh)
	return forwardmodel(ks, rs)
end

#constructs qubo for the inverse problem and solves it
#inputs: heads, low and high permeability, recharges
#output: binary permeabilities and matrix representation of the qubo
...
#inputs: binary representation of the permeability, observation standard deviation, low and high permeability
#outputs: binary representation of the permeability, recharges, "observed" heads (with noise)
function getrealization(truekb, sigmaobs, klow, khigh)
	truers = zeros(length(truekb) - 1)
	trueh = forwardmodel(truekb, truers, klow, khigh)
	obsh = trueh + sigmaobs * randn(length(trueh))
	return truekb, truers, obsh
end
...
klow = 1.0
sigmaobs = 0.0
#make sure the embedding is legit
...
for (j, khigh) in enumerate([2, 10, 100])
	...
	@time for i = 1:sample_size
		...
		truekb, truers, obsh = getrealization(truekb, sigmaobs, klow, khigh)
